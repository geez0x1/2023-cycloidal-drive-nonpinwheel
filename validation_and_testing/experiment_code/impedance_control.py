...
def main(stdscr):
	...
	# Impedance gains
	K = 30		# Stiffness (at the output) [Nm/rad]
	D = 3		# Damping (at the output) [Nm s/rad]

	# Control gains
	# Increase the following control gains as appropriate to get
	# very stiff position control for this test.
	vel_gain		= (D * 2*math.pi) / (n**2 * k_t)	            # [Nm / turns/s] - default 0.16
	pos_gain		= (K * 2*math.pi) / (n**2 * k_t * vel_gain)    # [turns/s / turn] - default 20
	vel_integrator_gain = 0     # [Nm / turn/s^2] - default 0.32
	...
	out_filename = get_output_filename('out_impedance_control.csv', stdscr)
	...
	# Debug
	stdscr.addstr(line,0,'pos_gain = '+str(pos_gain)+', vel_gain = '+str(vel_gain)+', product = '+str(pos_gain*vel_gain)); line += 1
