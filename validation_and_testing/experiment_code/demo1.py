...
# Total inertia, scaled to current 
def totalInertia_toCurrent():
	# Load inertia and approximate motor inertia
	I_l = loadInertia_light() / n**2
	I_m = 0.1 * 0.04**2
	
	# Torque constant
	k_t = 0.083
	
	# Return inertia scaled by torque constant (ODrive torque constant=1)
	# Also converted to turns/s^2
	return turns2rad * (I_m + I_l) / k_t
...
inertia			= totalInertia_toCurrent() # Inertia converted to current

# Motion behaviour
max_speed		= 30	# Max output speed [rad/s]
max_accel		= 140	# Max acceleration/deceleration [rad/s^2]
target_pos		= [math.pi, 1.5*math.pi, 0.5*math.pi, 2*math.pi] # Target positions [rad]
static_time		= 0.00	# How long to stand still between positions [s]
reached_margin	= 0.01	# Acceptable error for reaching setpoint [rad], at output
max_reach_time	= 5.0	# Maximum time allowed to reach the setpoint [s]

# Control gains
pos_gain			= 15*20		# [turns/s / turn] - default 20
vel_gain			= 7*0.16	# [Nm / turns/s] - default 0.16
vel_integrator_gain	= 4*0.32	# [Nm / turn/s^2] - default 0.32
...
# Set encoder and current control bandwidth
my_drive.axis0.encoder.config.bandwidth = 2000
my_drive.axis0.motor.config.current_control_bandwidth = 2000
...
while True:
	...
	if not motion_setpoint_reached:
		...
		elif time.monotonic() - t_motion_setpoint_reached > max_reach_time:
			print('Failed to reach setpoint, aborting...')
			break
