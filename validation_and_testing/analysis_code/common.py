...
def filter_iir(data, alpha=None, f=None, dt=None):
	if not alpha and not f:
		raise Exception('filter_iir: Either alpha or f needs to be supplied')

	if f and not dt:
		raise Exception('filter_iir: Provide dt when providing f')

	# Compute alpha based on cut-off frequency
	if f:
		alpha = np.exp(-dt*2*np.pi*f)
		#print('filter_iir: Computed tau='+str((-dt/np.log(alpha)))+' and alpha='+str(alpha))

	# Allocate and set first item based on first input (avoid ugly convergence at start)
...
# Curve fitting function for bidirectional linear fit
def fit_linear_bidirectional(tau, a, b, c, d):
	return  np.heaviside(tau, 1) * (a * tau + b) + \
	        np.heaviside(-tau, 0) * (c * tau + d)
