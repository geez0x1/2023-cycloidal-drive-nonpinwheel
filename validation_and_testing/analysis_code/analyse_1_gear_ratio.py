...
# Filter data
motorPos_filt	= filter_iir(motorPos, f=5, dt=dt)
outputPos_filt	= filter_iir(outputPos, f=5, dt=dt)
motorVel_filt	= filter_iir(motorVel, f=5, dt=dt)
outputVel_filt	= filter_iir(outputVel, f=5, dt=dt)

# Compute the position inaccuracy, removing DC offset
# We can use filtering here to clean up data
diff = (motorPos_filt/n) - outputPos_filt
...
# Compute RMS error
# When using this value, do check if it is not startup behaviour...
e = turns2deg*diff
rmse = np.sqrt(e.dot(e)/e.size)
maxe = np.max(np.abs(e))
print('RMS error:', round(rmse,5), 'deg, max abs:', round(maxe,5), 'deg.')
